...
def register_mirror_tools(mcp: FastMCP) -> None:
    ...
    async def raw_search(
        ...
        Bypasses the curated name-resolution and shaping of `search_used_cars`;
        you supply V1 wire params directly (e.g. `marka_id[0]`, `category_id`,
        `s_yers[0]`). Cached briefly (volatile). Returns the raw API response.
        """
        async with tool_errors():
            rt = get_runtime()
            key = make_cache_key("/auto/search", params)
            cached = await rt.volatile_cache.get(key)
            if cached is not None:
                return cached
            raw = await rt.client.get_json("/auto/search", params)
